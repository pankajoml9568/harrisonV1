...
class PostRepository
{
    ...
    public function storeImageData($request)
    {
        ...
        if ($photos = $request->file('photo')) {
            foreach($photos as $key => $photo){
                $storeData[$key] = 'harrison/'.ImageHelper::handleUploadedImage($photo,'assets/images/harrison');
            }
        }
        return $storeData;
    }
    ...
    public function UpdateImageData($request,$post)
    {
        ...
        if ($photos = $request->file('photo')) {
            foreach($photos as $key => $photo){
                array_push($storeData,'harrison/'.ImageHelper::handleUploadedImage($photo,'assets/images/harrison'));
            }
        }
